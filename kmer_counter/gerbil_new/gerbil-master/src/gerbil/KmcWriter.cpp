		...
		if(_outputFormat == of_fasta) {
			...
			while(_kmcSyncSwapQueue->swapPop(kb)) {
				...
				if(!kb->isEmpty()) {
					...
					// ofstream bubble_fwrite_run2;
					// bubble_fwrite_run2.open("bubble_fwrite_run2.csv", ios::app);
					// bubble_fwrite_run2 << timer.tv1.tv_sec << "," << timer.tv1.tv_usec << endl; // start time
					// bubble_fwrite_run2 << timer.tv2.tv_sec << "," << timer.tv2.tv_usec << endl; // end time
					// bubble_fwrite_run2.close();
