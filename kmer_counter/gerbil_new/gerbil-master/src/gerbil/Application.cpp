...
void gerbil::Application::parseParams(const int &argc, char** argv) {
	// printParamsInfo();
	// printf("application.cpp::_sequenceSplitterThreadsNumber: %d\n",_sequenceSplitterThreadsNumber);
	uint i;
...
void gerbil::Application::process(const int &argc, char** argv) {
	...
	if(std::remove("bubble_fread_run1.csv")==0)
 		cout << "previous bubble_fread_run1.csv removed ..." << endl;

 	if(std::remove("bubble_fwrite_run1.csv")==0)
 		cout << "previous bubble_fwrite_run1.csv removed ..." << endl;

	if(std::remove("bubble_fread_run2.csv")==0)
 		cout << "previous bubble_fread_run2.csv removed ..." << endl;

 	if(std::remove("bubble_fwrite_run2.csv")==0)
 		cout << "previous bubble_fwrite_run2.csv removed ..." << endl;

 	if(std::remove("ukmerFile.csv")==0)
 		cout << "previous ukmerFile.csv removed ..." << endl;
 	
 	if(std::remove("mmer_file_mapping.csv")==0)
 		cout << "previous mmer_file_mapping.csv removed ..." << endl;
...
#define SET_DEFAULT(x, d) if(!x) x = d
	...
	if (!_m) {
		...
		// test
		// _m = 6;
		// printf("Application.cpp::(((uint64) 4) << (2 * _m)): %d\n ",(((uint64) 4) << (2 * _m)) );
...
#define CHECK_PARAM_IN_RANGE(x, min, max, s1, s2)																			\
	...
	// printf("Application.cpp::_m: %d, (((uint64) 4) << (2 * _m)):%zu \n",_m,(((uint64) 4) << (2 * _m)));
...
void gerbil::Application::saveUKmerStat(){

	ofstream ukmerFile;
	ukmerFile.open("ukmerFile.csv", ios::app);
	for (uint_tfn tempFileId(0); tempFileId < _tempFilesNumber; ++tempFileId){
		ukmerFile << tempFileId << "," <<  _tempFiles[tempFileId].getUKMersNumber() << endl;
	}
	ukmerFile.close();
	

	// std::string UKmerFileName(_tempFolderName);
	// UKmerFileName += "UKmerStatFile.txt";
	// FILE* UKmerStatFile = fopen(UKmerFileName.c_str(), "wb+");
	// for (uint_tfn tempFileId(0); tempFileId < _tempFilesNumber; ++tempFileId){
	// 	fprintf(file,"%lu,%lu",tempFileId,_tempFiles[tempFileId]._ukmers);
	// 	// _tempFiles[tempFileId].fprintStat(UKmerStatFile);
	// }
