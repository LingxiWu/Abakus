...
void gerbil::SequenceSplitter::detMMerHisto() {
	...
	// printf("SequenceSplitter.cpp:: _m: %d, mMersNumber: %d\n", _m, mMersNumber);

	// kmc2 strategy
	std::vector<std::pair<uint32_t,uint32_t>> kMerFrequencies;
	uint32 numAllowed = 0;
	for(uint32 mmer = 0; mmer < mMersNumber; ++mmer){
		// printf("sequenceSplitter.cpp:: mmer: %zu, isAllowed: %zu\n", mmer, isAllowed(mmer));
		if(isAllowed(mmer)){
			numAllowed += 1;
		}
		kMerFrequencies.push_back(std::make_pair(isAllowed(mmer) ? mmer : 0xffffffffu, mmer));

		// use the natural order of A<C<G<T, with exceptions.
		// sort on it->first (order/ranking), it->second (mmer)
		// allowed mmer are at the front, in their natural A<C<G<T order
		
	}
	// printf("SequenceSplitter.cpp::numAllowed: %d\n",numAllowed);

	std::sort(kMerFrequencies.begin(), kMerFrequencies.end());
	uint32 rank = 0;
	// _mVal[mmer] --> mmer order/ranking lookup
	// _mVal[mmer] = ranking or that mmer
	for (std::vector<std::pair<uint32_t,uint32_t>>::iterator it = kMerFrequencies.begin() ; it != kMerFrequencies.end(); ++it) {
		...
		// printf("sequenceSplitter.cpp::it->first: %zu, it->second: %zu\n", it->first, it->second);
		// printf("it->second (mmer): %zu, _mVal[it->second] (ranking): %zu\n", it->second, _mVal[it->second]);
	...
	int32 p = mMN / bfnx2;
	// printf("SequenceSplitter.cpp::_tempFilesNumber: %zu, mMersNumber: %zu, bfnx2: %zu, mMersNumber % bfnx2: %zu, mMN: %zu, p: %zu\n",
		// _tempFilesNumber, mMersNumber, bfnx2, mMersNumber % bfnx2, mMN, p);
	// printf("SequenceSplitter.cpp:: _tempFilesNumber: %d, mMersNumber: %d\n",_tempFilesNumber,mMersNumber);
	uint32 count = 0;
	for(uint32 i = 0; i < mMersNumber; ++i){
		// printf("SequenceSplitter.cpp::_mVal[%zu]: %zu\n",i, _mVal[i]);
		if(_mVal[i] < mMN){
			// count ++;
			// printf("sequenceSplitter.cpp:: _mVal[%zu]: %zu, mMN: %zu\n",i, _mVal[i],mMN);
			int32 d = _mVal[i] / bfnx2;
			int32 m = (_mVal[i] + d * bfnx2 / p) % bfnx2;
			int32 x = m - (int32)_tempFilesNumber;
			_mToBin[i] = x < 0 ? -x - 1 : x;
			// if(_mVal[i] == 123 || _mVal[i] == 337 || _mVal[i] == 1023){
			// 	printf("sequenceSplitter.cpp:: d: %zu, m: %zu, x: %zu\n", d,m,x);
			// }
		...
		ofstream mmerFileMap;
		mmerFileMap.open("mmer_file_mapping.csv", ios::app);		
		mmerFileMap << i << "," << _mVal[i] << "," << isAllowed(i) << "," << _mToBin[i] << endl;		
		mmerFileMap.close();
		
	}
	// printf("SequenceSplitter.cpp:: count: %zu\n",count);
