...
void gerbil::FastParser::processFastq(const size_t &tId) {
	...
	while (true) {
		...
		SimpleTimer timer; // bubble timer
		nextPart(bp, bp_end, tId);
		...
		while (*bp) {
			...
			// ofstream bubbleFile_in;
			// bubbleFile_in.open("bubble_in_run1.csv", ios::app);
			// bubbleFile_in << timer.tv1.tv_sec << "," << timer.tv1.tv_usec << endl; // timer bubble
			// bubbleFile_in << timer.tv2.tv_sec << "," << timer.tv2.tv_usec << endl; // timer bubble
			// bubbleFile_in.close();
...
void gerbil::FastParser::process() {
	...
	for (uint32_t i = 0; i < _threadsNumber; i++) {
		_processThreads[i] = new std::thread([this](uint64_t tId) {
			...
			switch (_fileType) {
				case ft_fastq:
					// printf("FastParser process Fastq. tId: %u \n", tId);
					processFastq(tId);
					break;
				case ft_fasta:
					processFasta(tId);
